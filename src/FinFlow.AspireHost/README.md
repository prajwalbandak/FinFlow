...
1. Set `FinFlow.AspireHost` as the startup project
...
```bash
dotnet ef migrations add MigrationName -c AppDbContext -p ../FinFlow.Infrastructure/FinFlow.Infrastructure.csproj -s FinFlow.Web.csproj -o Data/Migrations
```

To update the database:
```bash
dotnet ef database update -c AppDbContext -p ../FinFlow.Infrastructure/FinFlow.Infrastructure.csproj -s FinFlow.Web.csproj
```
